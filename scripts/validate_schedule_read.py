...
Examples:
  ...
  python3 solem_blip_ble/scripts/validate_schedule_read.py --capture --verbose
  python3 scripts/validate-solem-schedule-read.py --replay btsnoop/captures/solem-schedule-*.jsonl
"""
...
def _repo_root() -> Path:
    ...
    if (script_root / "solem_blip_ble").is_dir():
        return script_root
    try:
        import solem_blip_ble

        return Path(solem_blip_ble.__file__).resolve().parents[2]
    except ImportError:
        return script_root
...
def _default_capture_output() -> Path:
    ...
    return _repo_root() / "btsnoop" / "captures" / f"solem-schedule-{stamp}.jsonl"
