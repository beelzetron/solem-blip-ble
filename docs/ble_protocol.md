...
| 10 | **Battery Voltage** | Raw 9 V reading (MySOLEM `statusAnswerV5`, byte 10) |
...
BL-IP reports raw battery voltage at **byte 10** of seq=0x02 status frames (MySOLEM `BluetoothV5FrameManager.statusAnswerV5`).

```python
battery_voltage = notification[10]  # 0 = not reported
```

Map to icon level 0–5 using 9 V thresholds `{60, 65, 70, 75, 80}`. Alert below **50** (MySOLEM `PowerSourceType.nineVolts`).
...
- ✅ Battery voltage at **byte 10** (MySOLEM `statusAnswerV5`, HCI capture `0x4f` → level 4)
